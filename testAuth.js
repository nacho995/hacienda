...
// Función para comparar contraseñas directamente con bcrypt
const comparePasswords = async (plainPassword, hashedPassword) => {
  try {
    ...
    console.log('Resultado de comparación bcrypt:', isMatch ? 'COINCIDE ✅' : 'NO COINCIDE ❌');
    
    // Probar a generar un nuevo hash y compararlo para ver si bcrypt funciona correctamente
...
mongoose.connect(process.env.MONGODB_URI)
  .then(async () => {
    ...
    try {
      ...
      // Generar un hash de la contraseña para forzar la actualización
      console.log('\nGenerando un nuevo hash para admin123...');
      const salt = await bcrypt.genSalt(10);
      ...
      // Probar la autenticación otra vez
      console.log('\nProbando autenticación después de la actualización...');
      const isMatch = await comparePasswords('admin123', updatedUser.password);
      
      if (isMatch) {
        console.log('✅ Autenticación exitosa después de la actualización');
      } else {
        console.log('❌ Autenticación fallida incluso después de la actualización');
      }
